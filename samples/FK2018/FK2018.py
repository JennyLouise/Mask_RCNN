"""
Mask R-CNN
Configurations and data loading code for FK2018 data in the MS COCO format.

Adapted from coco.py
Copyright (c) 2017 Matterport, Inc.
Licensed under the MIT License (see LICENSE for details)
Written by Waleed Abdulla
Adapted by Jenny Walker
...
Usage: import the module (see Jupyter notebooks for examples), or run from
    ...
    python3 FK2018.py train --dataset=/path/to/coco/ --model=coco

    # Train a new model starting from ImageNet weights. Also auto download COCO dataset
    python3 FK2018.py train --dataset=/path/to/coco/ --model=imagenet --download=True

    # Continue training a model that you had trained earlier
    python3 FK2018.py train --dataset=/path/to/coco/ --model=/path/to/weights.h5

    # Continue training the last model you trained
    python3 FK2018.py train --dataset=/path/to/coco/ --model=last

    # Run COCO evaluatoin on the last model you trained
    python3 FK2018.py evaluate --dataset=/path/to/coco/ --model=last
"""
...
class FKConfig(Config):
    ...
    NUM_CLASSES = (
        10
    )  # Full tunasand set has 33, Large Megafauna tunasand set has 10, COCO has 80 classes
    STEPS_PER_EPOCH = 15
    BATCH_SIZE = 32
...
class FKDataset(utils.Dataset):
    def load_fk(self, dataset_dir, class_ids=None, class_map=None, return_coco=False):
        ...
        # if class ids passed as an argument
        # only process images containing given classes
        if class_ids:
            image_ids = []
            for id in class_ids:
        ...
        # else, process all images & all class ids
        else:
...
# Function used for recursive label improvement - converts results of model inference to labelme files
